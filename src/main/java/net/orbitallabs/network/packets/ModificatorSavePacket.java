...
public class ModificatorSavePacket implements IMessage {
	...
	public ModificatorSavePacket()
	{
	}
	
	public ModificatorSavePacket(Structure object, List<Structure> addObj, List<Structure> childObj, BlockPos pos)
	{
		this.object = object;
		this.addObjects = addObj;
		this.ChildObjects = childObj;
		this.tilePos = pos;
	}
	...
	public static class Handler implements IMessageHandler<ModificatorSavePacket, IMessage> {
		@Override
		public IMessage onMessage(ModificatorSavePacket pkt, MessageContext ctx)
		{
			...
			if (pkt.tilePos != null)
			{
				...
				if (world.getTileEntity(pkt.tilePos) != null && world.getTileEntity(pkt.tilePos) instanceof TileEntityInfo)
				{
					...
					if (pkt.object != null && pkt.addObjects != null && pkt.ChildObjects != null)
					{
						pkt.object.connections = info.Object.connections;
						info.Object = pkt.object;
						
						info.ChildObjects = pkt.ChildObjects;
						info.AddObjects = pkt.addObjects;
						info.markDirty();
					}
				}
			}
