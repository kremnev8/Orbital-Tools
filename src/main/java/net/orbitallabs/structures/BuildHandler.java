...
public class BuildHandler {
								...
								if (wantedItem != null && curr != null)
								{
									if (wantedItem.isStackEqual(curr, true))
									{
										if (curr.example.getCount() >= wantedItem.example.getCount())
										{
											...
											if (inv.getStackInSlot(j) != null && inv.getStackInSlot(j).getCount() == 0)
											{
					...
					if (wantedItem != null && curr != null)
					{
						if (wantedItem.isStackEqual(curr, true))
						{
							if (curr.example.getCount() >= wantedItem.example.getCount())
							{
								...
								if (player.inventory.getStackInSlot(j) != null && player.inventory.getStackInSlot(j).getCount() == 0)
								{
	...
	public static void buildBuildPoint(World world, int x, int y, int z, int type)
	{
		...
		setBlock(world, x, y, z, info, type, 2);
	}
