...
public class GuiModificator extends GuiModular implements ISlider {
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		/*
		 * int oldX = guiLeft; int oldY = guiTop; int oldW = xSize; int oldH = ySize;
		 * 
		 * guiLeft = cornerX; guiTop = cornerY; xSize = realWidth; ySize = realHeight;
		 ...
		 * }
		 * 
		 * GlStateManager.enableBlend(); GlStateManager.translate((float) guiLeft,
		 * (float) guiTop, 0.0F); GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
		 * 
		 * this.drawGuiContainerForegroundLayer(mouseX, mouseY);
		 * 
		 * RenderHelper.enableStandardItemLighting(); guiLeft = oldX; guiTop = oldY;
		 * xSize = oldW; ySize = oldH;
		 */
	}
	...
	public void initGui() {
		...
		int max = (ChildObjects != null ? ChildObjects.size() * 4 : 0)
				+ (addObjects != null ? addObjects.size() * 4 : 0) - 4;

		this.buttonList.add(slider = new GuiVerticalSlider(0, x + 138, y + 15, 10, 141, "R", "R", 0, max,
				lastMove > max ? max : lastMove, true, false, this));
		...
		int length = fontRenderer.getStringWidth(I18n.format("modificator.save.name"));

		this.buttonList.add(new SmallGuiButton(1, x + 100 - (length + 12 < 50 ? 0 : length - 50), y + 159,
				(length <= 30 ? 40 : length + 12), I18n.format("modificator.save.name")));

		if (object != null) {
			this.buttonList.add(new GuiButtonModificator(2, x + 8, y + 16, object, y, true));
			// this.buttonList.add(new GuiMLabel(-1, x + 7, y + 59,
			// StatCollector.translateToLocal("modificator.children.name"), y));
			int lasti = 71;
			if (ChildObjects != null && ChildObjects.size() > 0) {
				for (int i = 0; i < ChildObjects.size(); i++) {

					this.buttonList.add(new GuiButtonModificator(buttonList.size(), x + 8, y + 71 + 40 * i,
							ChildObjects.get(i), y, false));
					lasti += 40;
			...
			if (addObjects != null && addObjects.size() > 0) {
				for (int i = 0; i < addObjects.size(); i++) {
					this.buttonList.add(
							new GuiButtonModificator(buttonList.size(), x + 8, y + lasti, addObjects.get(i), y, false));
					lasti += 40;
	...
	protected void drawGuiContainerBackgroundLayer(float time, int mX, int mY) {
		...
		if (NyPos > y - 25 || NyPos < y + 157) {
			fontRenderer.drawString(I18n.format("modificator.children.name"),
					x + 7 + (int) (133 / 2)
							- (fontRenderer.getStringWidth(I18n.format("modificator.children.name")) / 2),
					NyPos, 4210752, false);
		}

		NyPos = text2y + 3 - (10 * move);
		if (NyPos > y - 25 || NyPos < y + 157) {
			fontRenderer.drawString(I18n.format("modificator.additional.name"),
					x + 7 + (int) (133 / 2)
							- (fontRenderer.getStringWidth(I18n.format("modificator.additional.name")) / 2),
					NyPos, 4210752, false);
		}
	...
	protected void drawGuiContainerForegroundLayer(int MouseX, int MouseY) {
		fontRenderer.drawString(I18n.format("modificator.name"),
				(int) (xSize / 4.5D) - (fontRenderer.getStringWidth(I18n.format("modificator.name")) / 2) + 15, 6,
				4210752, false);
