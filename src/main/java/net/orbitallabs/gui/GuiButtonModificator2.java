...
public class GuiButtonModificator2 extends GuiButton {
	...
	public void drawButton(Minecraft mine, int x, int y)
	{
		...
		NyPos = this.yPosition - (10 * GuiModificator.move);
		if (visSelf)
		{
		...
		if (this.visible)
		{
			...
			this.hovered = x >= this.xPosition && y >= NyPos && x < this.xPosition + this.width && y < NyPos + this.height && !GuiButtonModificator.isAnyInFocus && !covered;
			...
			{
				this.drawTexturedModalRect(this.xPosition, NyPos, 0, 236, 129, 15);//en
			} else
			{
				this.drawTexturedModalRect(this.xPosition, NyPos, 0, 221, 129, 15);
			}
	...
	{
		boolean covered = y > ZeroPos + 157;
		return this.enabled && this.visible && x >= this.xPosition && y >= NyPos && x < this.xPosition + this.width && y < NyPos + this.height && !GuiButtonModificator.isAnyInFocus
				&& !covered;
	}
