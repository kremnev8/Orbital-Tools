		...
		if (b == BlockContainerMod.BlockRemoveInfo && !player.isSneaking()) {
			if (!world.isRemote) {
				...
				if (te != null && te.infoBlocks != null && te.infoBlocks.size() > 0) {
					...
					player.sendMessage(new TextComponentString(te.infoBlocks.get(0).getPos().getX() + " "
							+ te.infoBlocks.get(0).getPos().getY() + " " + te.infoBlocks.get(0).getPos().getZ()));
					...
					if (teI.Object != null && teI.Object.placementDir != null) {
						...
						player.sendMessage(new TextComponentString("POS:" + teI.Object.placementPos.getX() + " "
								+ teI.Object.placementPos.getY() + " " + teI.Object.placementPos.getZ()));
						if (teI.Object.connections != null && teI.Object.connections.size() > 0) {
							...
							for (int i = 0; i < teI.Object.connections.size(); i++) {
								player.sendMessage(
										new TextComponentString(teI.Object.connections.get(i).getUnlocalizedName() + " "
												+ teI.Object.connections.get(i).placementRotation + " "
												+ teI.Object.connections.get(i).placementDir.toString()));
							}
						}
					}

					if (te.infoBlocks.get(0).Object != null
							&& te.infoBlocks.get(0).Object.getUnlocalizedName() == Structure.BIGHHALL) {
						...
						if (ChildObjects != null && ChildObjects.size() > 0) {
							...
							for (int i = 0; i < ChildObjects.size(); i++)
								player.sendMessage(new TextComponentString(ChildObjects.get(i).getUnlocalizedName()
										+ " " + ChildObjects.get(i).placementRotation));
						}
						if (AddObjects != null && AddObjects.size() > 0) {
							...
							for (int i = 0; i < AddObjects.size(); i++)
								player.sendMessage(new TextComponentString(AddObjects.get(i).getUnlocalizedName() + " "
										+ AddObjects.get(i).placementRotation));
						}
					} else {
						...
						if (teI.ChildObjects != null && teI.ChildObjects.size() > 0) {
							...
							for (int i = 0; i < teI.ChildObjects.size(); i++)
								player.sendMessage(new TextComponentString(teI.ChildObjects.get(i).getUnlocalizedName()
										+ " " + teI.ChildObjects.get(i).placementRotation));
						}
						if (teI.AddObjects != null && teI.AddObjects.size() > 0) {
							player.sendMessage(new TextComponentString("Additional Structures:"));
							for (int i = 0; i < teI.AddObjects.size(); i++)
								player.sendMessage(new TextComponentString(teI.AddObjects.get(i).getUnlocalizedName()
										+ " " + teI.AddObjects.get(i).placementRotation));
						}
					}
			...
			if (world.isRemote) {
				...
				// {
				player.openGui(OrbitalTools.instance, GuiHandler.MODIFICATORGUI, world, pos.getX(), pos.getY(),
						pos.getZ());
				PacketHandler.sendToServer(
						new OpenGuiOnServerPacket(GuiHandler.MODIFICATORGUI, pos.getX(), pos.getY(), pos.getZ()));
				// } else ChatUtils.SendChatMessageOnClient(player, ChatUtils.modifyColor(new
				// TextComponentString(I18n.format("modificator.notready.name")),
				// TextFormatting.RED));
			}
		}
