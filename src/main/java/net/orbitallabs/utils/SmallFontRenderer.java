...
public class SmallFontRenderer implements IResourceManagerReloadListener {
	...
	private ResourceLocation func_111271_a(int par1)
	{
		...
		{
			SmallFontRenderer.field_111274_c[par1] = new ResourceLocation(String.format("textures/font/unicode_page_%02x.png", new Object[] { Integer.valueOf(par1) }));
		}
		...
		for (int i = 0; i < par1Str.length(); ++i)
		{
			...
			} else
			{
				...
				if (this.strikethroughStyle)
				{
					...
					vertexbuffer.pos(this.posX + f1, this.posY + this.FONT_HEIGHT / 2, 0.0D).endVertex();
					vertexbuffer.pos(this.posX + f1, this.posY + this.FONT_HEIGHT / 2 - 1.0F, 0.0D).endVertex();
				...
				if (this.underlineStyle)
				{
					...
					vertexbuffer.pos(this.posX + l, this.posY + this.FONT_HEIGHT, 0.0D).endVertex();
					vertexbuffer.pos(this.posX + f1, this.posY + this.FONT_HEIGHT, 0.0D).endVertex();
					vertexbuffer.pos(this.posX + f1, this.posY + this.FONT_HEIGHT - 1.0F, 0.0D).endVertex();
					vertexbuffer.pos(this.posX + l, this.posY + this.FONT_HEIGHT - 1.0F, 0.0D).endVertex();
					tessellator.draw();
					GL11.glEnable(GL11.GL_TEXTURE_2D);
				}
	...
	String wrapFormattedStringToWidth(String par1Str, int par2)
	{
		...
		} else
		{
			...
			String s2 = SmallFontRenderer.getFormatFromString(s1) + par1Str.substring(j + (flag ? 1 : 0));
